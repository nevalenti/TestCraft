...
public class TestResultRepository(AppDbContext db) : ITestResultRepository
{
    public async Task<IEnumerable<TestResultDto>> GetAllAsync(Guid projectId, Guid runId, CancellationToken cancellationToken = default)
        => await db.TestResults
            ...
            .Select(r => new TestResultDto(r.Id, r.TestRunId, r.TestCaseId, r.TestCase.Name, r.Status, r.Notes, r.ExecutedAt, r.ExecutedById, r.CreatedAt, r.UpdatedAt))
    ...
    public async Task<TestResultDto> AddAsync(TestResult result, CancellationToken cancellationToken = default)
    {
        ...
        var testCaseName = await db.TestCases
            .Where(c => c.Id == result.TestCaseId)
            .Select(c => c.Name)
            .FirstAsync(cancellationToken);

        return new TestResultDto(result.Id, result.TestRunId, result.TestCaseId, testCaseName, result.Status, result.Notes, result.ExecutedAt, result.ExecutedById, result.CreatedAt, result.UpdatedAt);
    }
