...
{
    public async Task<IEnumerable<TestCaseDto>> GetAllAsync(Guid projectId, Guid suiteId, string? search = null, CancellationToken cancellationToken = default)
        => await db.TestCases
            .AsNoTracking()
            .Where(c => c.SuiteId == suiteId && c.Suite.ProjectId == projectId)
            .Where(c => search == null || c.Name.ToLower().Contains(search.ToLower()))
            .Select(c => new TestCaseDto(c.Id, c.SuiteId, c.Name, c.Description, c.Priority, c.Steps.Count(s => !s.IsDeleted), c.CreatedAt, c.UpdatedAt))
    ...
    public async Task<TestCaseDto> AddAsync(TestCase testCase, CancellationToken cancellationToken = default)
    {
        ...
        return new TestCaseDto(testCase.Id, testCase.SuiteId, testCase.Name, testCase.Description, testCase.Priority, 0, testCase.CreatedAt, testCase.UpdatedAt);
    }
