...
public class TestSuitesApiTests(ApiFactory factory)
{
    ...
    public async Task Create_Then_GetById_ReturnsSuite()
    {
        ...
        var created =
            await createResponse.Content.ReadFromJsonAsync<TestSuiteResponse>();
        ...
        var fetched =
            await getResponse.Content.ReadFromJsonAsync<TestSuiteResponse>();
    ...
    public async Task GetAll_FiltersBySearch()
    {
        ...
        var response = await client.GetAsync(
            $"/api/v1/projects/{project.Id}/suites?search=Login"
        );
        response.StatusCode.Should().Be(HttpStatusCode.OK);

        var page = await response.Content.ReadFromJsonAsync<
            Paginated<TestSuiteResponse>
        >();
    ...
    public async Task Update_ChangesNameAndDescription()
    {
        ...
        var updated =
            await updateResponse.Content.ReadFromJsonAsync<TestSuiteResponse>();
    ...
    public async Task GetById_OtherUsersProject_ReturnsNotFound()
    {
        ...
        var suite = await ownerClient.CreateSuiteAsync(
            project.Id,
            "Private Suite"
        );
