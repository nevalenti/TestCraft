...
public class ProjectTestCasesApiTests(ApiFactory factory)
{
    ...
    public async Task GetAll_ReturnsCasesAcrossSuitesInProject()
    {
        ...
        var loginSuite = await client.CreateSuiteAsync(
            project.Id,
            "Login Suite"
        );
        var checkoutSuite = await client.CreateSuiteAsync(
            project.Id,
            "Checkout Suite"
        );

        await client.CreateCaseAsync(
            project.Id,
            loginSuite.Id,
            "Successful login"
        );
        ...
        var response = await client.GetAsync(
            $"/api/v1/projects/{project.Id}/cases"
        );
    ...
    public async Task GetAll_FiltersBySearch()
    {
        ...
        var response = await client.GetAsync(
            $"/api/v1/projects/{project.Id}/cases?search=login"
        );
        response.StatusCode.Should().Be(HttpStatusCode.OK);
