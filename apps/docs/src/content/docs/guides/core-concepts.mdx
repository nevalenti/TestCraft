---
title: Core Concepts
description: The entities TestCraft is built around — projects, suites, cases, runs, plans, analytics, and notifications.
sidebar:
  order: 2
---
...
The mental model behind the web app: what each entity is, how they relate,
and where to configure them. Read this once you're past [Getting
Started](/docs/guides/getting-started/) and need to know what you're looking
at. For reporting results _from_ CI, see [CI
Integration](/docs/guides/ci-integration/).
...
<Aside type="note">
  Plans don't have a nav tab yet — reach them at
  `/projects/<project-id>/plans`.
</Aside>

## Test runs

A run tracks pass/fail/blocked/skipped results for a set of test cases in one
environment. Create one manually (**New Run**), from a test plan, or from CI
(the GitHub Action, or the `testcraft-ci-reporter` CLI's `start` command).
...
| Flaky Tests     | Cases alternating pass/fail, ranked by flake rate (high ≥60%, medium 30–60%, low under 30%) |
...
- **API Tokens** — create and revoke named, optionally expiring tokens.

  <Aside type="danger">
    Tokens are shown once, at creation, and can't be recovered afterward.
  </Aside>

## Account

The top-right avatar leads to your profile, photo upload, and Keycloak's
account console for editing your name, email, and password.
