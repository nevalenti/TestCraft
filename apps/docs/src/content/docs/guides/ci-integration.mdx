...
Both reporters authenticate against Keycloak as a TestCraft **service
account**, then call the API with the resulting token. Create the service
account in Keycloak first and give it access to the relevant project. Both
reporters take:
...
| ----------------------- | ------------------------------------------- | ----------------------------------------------------------------------------------- |
| `command`               | <Badge text="optional" variant="note" />    | `start` creates an Active run before tests; default `import` imports results after  |
| `username` / `password` | <Badge text="optional" variant="note" />    | Service account credentials                                                         |
| `api-url`               | <Badge text="optional" variant="note" />    | TestCraft API base URL — omitted makes the step a no-op                             |
| `project-name`          | <Badge text="required" variant="success" /> | Project to import results into                                                      |
| `junit-xml`             | <Badge text="optional" variant="note" />    | Path or single-directory glob to a JUnit XML report — required for `import`         |
| `run-name`              | <Badge text="required" variant="success" /> | Display name for the run                                                            |
| `keycloak-authority`    | <Badge text="optional" variant="note" />    | Public Keycloak authority — skips the `auth-config` lookup when set                 |
| `source`                | <Badge text="optional" variant="note" />    | Source tag for test suites (`api`, `web`, `e2e`, ...)                               |
| `screenshots-dir`       | <Badge text="optional" variant="note" />    | Playwright `test-results` directory — uploads PNGs as attachments to failed results |

</TabItem>
<TabItem label="CLI reporter">

For GitLab CI, Jenkins, or any other system. Build `testcraft-ci-reporter`
from source (the npm package isn't published yet):
