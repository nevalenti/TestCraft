...
- **Production** — `https://<your-domain>/api/docs`, proxied through the
  Gateway's `/api/*` route
...
`health`, `status`, and `auth-config` are fully anonymous, and `metrics` uses
its own separate `METRICS_TOKEN` bearer check instead of a Keycloak token.
Use the `bearerAuth` security scheme in Swagger UI to authenticate requests
directly from the docs.

<LinkCard
  title="CI / machine-to-machine access"
  href="/docs/guides/ci-integration/"
  description="Authenticate as a service account from GitHub Actions, GitLab CI, or Jenkins."
/>
