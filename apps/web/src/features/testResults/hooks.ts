...
import { testResultQueries, testResultsApi } from '@/features/testResults/api';
...
export const useCreateTestResult = (projectId: string, runId: string) => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      notify(`Result saved for "${data.testCaseName}": ${data.status}`);
...
export const useUpdateTestResult = (projectId: string, runId: string) => {
  ...
  return useMutation({
    ...
    onSuccess: (data, { id }) => {
      notify(`Result for "${data.testCaseName}" updated to ${data.status}`);
      queryClient.invalidateQueries({
        queryKey: queryKeys.testResults.all(projectId, runId),
      });
      queryClient.invalidateQueries({
