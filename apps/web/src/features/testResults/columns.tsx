...
import { DefectTypeBadge } from '@/features/testResults/DefectTypeBadge';
...
export const createColumns = ({
  ...
  columnHelper.display({
    ...
    cell: ({ row, table }) => {
      ...
      return (
        <span className="text-xs text-base-content/65 tabular-nums">
  ...
  columnHelper.accessor('notes', {
    ...
    cell: (info) => {
      ...
      return value ? (
        <div
          className="max-w-[200px] cursor-default truncate text-sm text-base-content/85"
          title={value}
        >
          {value}
        </div>
      ) : (
        <span className="text-sm text-base-content/55 italic">—</span>
      );
    },
  }),
