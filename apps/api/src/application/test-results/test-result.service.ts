...
import {
  CreateTestResult,
  UpdateTestResult,
} from '@/application/test-results/test-result.types';
...
export interface ITestResultService {
  ...
  create(
    runId: string,
    dto: CreateTestResult,
    userId?: string,
  ): Promise<TestResult>;
  update(
    runId: string,
    id: string,
    dto: UpdateTestResult,
...
export class TestResultService implements ITestResultService {
  ...
  async create(
    ...
    return this.testResultRepository.create(runId, dto, userId);
  }

  update(runId: string, id: string, dto: UpdateTestResult) {
    return this.testResultRepository.update(runId, id, dto);
  }
